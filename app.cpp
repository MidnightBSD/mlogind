...
bool App::AuthenticateUser(bool focuspass){
	...
	if (encrypted && strcmp(encrypted, correct) == 0)
		return true;
	sleep(cfg->getIntOption("wrong_passwd_timeout"));
	return false;
}
#endif
...
void App::Reboot() {
	...
	StopServer();
	RemoveLock();
	Util::run_command(cfg->getOption("reboot_cmd"));
	exit(OK_EXIT);
}
...
void App::Halt() {
	...
	StopServer();
	RemoveLock();
	Util::run_command(cfg->getOption("halt_cmd"));
	exit(OK_EXIT);
}

void App::Suspend() {
	sleep(1);
	Util::run_command(cfg->getOption("suspend_cmd"));
}


/*
 * Returns true if every printf conversion specifier in fmt is a plain
 * integer specifier (%d, %i, %u, %o, %x, %X) with only optional
 * decimal-digit width/precision modifiers.  %% is also accepted.
 * Anything else (%s, %p, %n, %l, ...) causes a return of false.
 */
static bool console_cmd_format_safe(const std::string &fmt)
{
...
void App::Console() {
	...
	/* Execute console */
	string cmd_template = cfg->getOption("console_cmd");
	if (!console_cmd_format_safe(cmd_template)) {
		logStream << APPNAME
			  << ": console_cmd contains unsafe format specifiers, refusing to execute"
			  << endl;
		return;
	}
	/* 6 integer substitutions, each at most 10 digits */
	vector<char> buf(cmd_template.size() + 6 * 10 + 1);
	snprintf(buf.data(), buf.size(), cmd_template.c_str(),
		 width, height, posx, posy, fontx, fonty);
	Util::run_command(buf.data());
}
...
void App::CreateServerAuth() {
	/* create mit cookie using cryptographically secure random bytes */
	uint8_t buf[App::mcookiesize / 2];
	const char *digits = "0123456789abcdef";
	arc4random_buf(buf, sizeof(buf));
	for (int i = 0; i < App::mcookiesize; i += 2) {
		mcookie[i]   = digits[buf[i/2] & 0x0f];
